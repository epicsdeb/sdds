 ...
 * Michael Borland, 1986 (after CITLIB routine MATINV)
 $Log: cm_invert3.c,v $
 Revision 1.2  2009/11/19 20:48:27  shang
 rewrote using the default "double complex" type in C99.
...
int cm_invert(CMATRIX *A, CMATRIX *B)         /* A=inv(B) */
{
  ...
  static double complex *pivot=NULL, piv, *tmp, t,swap;
  static double amax, abs_amax; 
  static long row, col, max_n=0;
  static double complex *a_j, *a_col, *a_m;
  ...
  for (i=0; i<n; i++) {
    ...
    for (j=0; j<n; j++) {    
      ...
      if (ipivot[j]!=1) {
        for (k=0; k<n; k++) {
          if (ipivot[k]>1)
            return(0);
          if (ipivot[k]!=1 && abs_amax<cabs(a_j[k])) {
            col = k;
            row = j;
            abs_amax = cabs(amax = a_j[k]);
          }
        }
      }
    }
    ...
    for (l=0; l<n; l++) {
      if (a_col[l]!=0 && ((long)log10(cabs(a_col[l]))-log10_piv > NDECADES)) {
    ...
    for (m=0; m<n; m++) {
      if (m!=col) {
        ...
        a_m[col] = 0.;
        if (t!=0)
          log10_t = log10(cabs(t))+0.5;
        else
          log10_t = 0;
        for (l=0; l<n; l++) {
          if ((a_col[l]!=0?(long)log10(cabs(a_col[l])+0.5):0)+log10_t > NDECADES) {

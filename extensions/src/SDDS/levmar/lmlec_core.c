 ...
 * one. See pp. 430-433 (chap. 15) of "Numerical Optimization" by Nocedal-Wright
 * for details
 *
            ...
            r_sz=mintmn*mintmn; /* // actually smaller if a is not of full row rank*/
              worksz=2*tn+(tn+1)*32; /* // more than needed*/
                ...
                /* //dgeqpf_((int *)&tm, (int *)&tn, a, (int *)&tm, jpvt, tau, work, &info);*/
                  ...
                  /* the upper triangular part of a now contains the upper triangle of the unpermutted R */
                  ...
                  tmp=tm*CNST(10.0)*eps*fabs(a[0]); /* // threshold. tm is max(tm, tn)*/
                    tmp=(tmp>CNST(1E-12))? tmp : CNST(1E-12); /* // ensure that threshold is not too small*/
                      ...
                      /* compute the permutted inverse transpose of R */
                                       ...
                                       */
{
  ...
  LMLEC_ELIM(A, b, data.c, NULL, Z, k, m); /* // compute c, Z*/
    ...
    for(i=0; i<m; ++i){
      ...
      for(j=0, tmp=data.c[i]; j<mm; ++j)
        tmp+=Zimm[j]*pp[j]; /* // tmp+=Z[i*mm+j]*pp[j];*/
          if(tmp!=p0[i])
            fprintf(stderr, RCAT("Warning: component %d of starting point not feasible in ", LEVMAR_LEC_DER) "()! [%g reset to %g]\n",
                    i, p0[i], tmp);
    }
    ...
    if(covar){
      TRANS_MAT_MAT_MULT(data.jac, covar, n, m, __BLOCKSZ__); /* covar = J^T J */
  ...
  LMLEC_ELIM(A, b, data.c, NULL, Z, k, m); /* // compute c, Z*/
    ...
    for(i=0; i<m; ++i){
      ...
      for(j=0, tmp=data.c[i]; j<mm; ++j)
        tmp+=Zimm[j]*pp[j]; /* // tmp+=Z[i*mm+j]*pp[j];*/
          if(tmp!=p0[i])
            fprintf(stderr, RCAT("Warning: component %d of starting point not feasible in ", LEVMAR_LEC_DIF) "()! [%g reset to %g]\n",
                    i, p0[i], tmp);
    }
    ...
    if(covar){
      ...
      FDIF_FORW_JAC_APPROX(func, p, hx, wrk, LM_DIFF_DELTA, jac, m, n, adata); /* compute the jacobian at p */
